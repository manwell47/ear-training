...
export class Visualizer {
  ...
  render() {
    try {
      ...
      // DEBUG OVERLAY FOR SAMPLE RATE AND CUTOFF
      if (this.audioEngine && this.audioEngine.ctx) {
        let maxFreqWithEnergy = 0;
        let nyq = this.audioEngine.ctx.sampleRate / 2;
        let powerSum = 0;
        if (this.fftBuffer) {
           for (let i = 0; i < this.fftBuffer.length; i++) {
              if (this.fftBuffer[i] > -100) {
                 maxFreqWithEnergy = (i / this.fftBuffer.length) * nyq;
              }
              powerSum += Math.pow(10, this.fftBuffer[i] / 10.0);
           }
        }
        let rms_dB = 10 * Math.log10(powerSum);
        if (rms_dB === -Infinity) rms_dB = -140;

        this.ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
        this.ctx.font = '12px monospace';
        this.ctx.textAlign = 'left';
        this.ctx.fillText(`SR: ${this.audioEngine.ctx.sampleRate} Hz | Nyq: ${nyq} Hz | MaxFreq: ${Math.round(maxFreqWithEnergy)} Hz | Power: ${rms_dB.toFixed(1)} dBFS`, 10, 20);
      }
  ...
  drawGrid() {
    ...
    // 3. RTA Absolute dBFS Scale (Left Side)
    const RTA_TOP_DBFS = 0.0;
      ...
      rtaSteps.forEach(db => {
        const norm = (db - RTA_BOTTOM_DBFS) / RTA_RANGE;
        const y = this.height - norm * this.height;
        let textYdB = y - 5;
        if (textYdB > this.height - 15) textYdB = this.height - 15; // Avoid colliding with bottom Hz labels

        let label = db === 0 ? `0 dBFS` : `${db} dBFS`;
        ctx.fillStyle = 'rgba(11, 12, 16, 0.85)';
        let textWidth = ctx.measureText(label).width;
        let finalY = db === 0 ? y + 12 : textYdB;
        ctx.fillRect(8, finalY - 10, textWidth + 4, 14);

        ctx.fillStyle = 'rgba(255, 255, 255, 0.4)'; 
        ctx.fillText(label, 10, finalY);
      });
  ...
  drawSpectrum() {
    ...
    if (this.rtaMode === 'raw') {
      ...
      let alpha = 0.03; // Suavizado Ultra Premium estilo Pro-Q (curvas más anchas)
      let s = rawY[0];
      for (let x = 0; x < width; x++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      // Attack (señal sube = Y baja) vs Release (señal baja = Y sube)
      if (y < this.previousY[x]) {
        ...
        // 1.5 dB per frame @ 60fps = 90 dB/sec
        let releaseDbPerFrame = isSynthetic ? 0.7 : 1.3; 
    ...
    // 1. Draw Peak Hold Trace (Back Layer)
    ctx.beginPath();
    for (let x = 0; x < width; x++) {
      if (x === 0) ctx.moveTo(x, this.peakHoldY[x]);
      else ctx.lineTo(x, this.peakHoldY[x]);
    }
    ctx.strokeStyle = isSynthetic ? "rgba(0, 210, 255, 0.25)" : "rgba(16, 185, 129, 0.25)";
    ctx.lineWidth = 1;
    ctx.stroke();
    ...
    if (isSynthetic) {
        ...
        gradFill.addColorStop(0.5, "rgba(0, 210, 255, 0.1)");
